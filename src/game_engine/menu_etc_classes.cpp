...
MsgLog::MsgLog(
	const RopeDeque& s_data, i32 s_internal_height,
	const IntVec2& s_window_size_2d, Vec2<bool> s_center, bool s_keep_sep
) {
...
Menu::Node::Node(
	const NoConn& s_most_args, const std::string& s_up,
...
Menu::Menu(
	const std::string& s_sel_key, const IntVec2& s_size_2d,
...
const std::string& Menu::_inner_next_sel_key(
	...
	auto temp_func = [&](const std::string& up_or_down)
		...
		if (
			(temp_node.kind == Node::Kind::ActionButton)
...
Menu::operator MsgLog() const {
	...
	for (
		const std::string* key=&at(Menu::START_NODE_KEY).down;
...
auto Menu::build_text_only_knc_pair(
	const std::string& key, const std::string& s_text
) -> KncPair {
	return {key, {.text=s_text, .kind=Menu::Node::Kind::TextOnly}};
}
...
auto Menu::build_node_map(const std::vector<KncPair>& vec)-> NodeMap {
	...
	//	for (
	//		std::string key=ret.at(START_NODE_KEY).down;
	//		key!=END_NODE_KEY;
	//		key=ret.at(key).down
	//	) {
	//		printout("\"", key, "\"; ",
